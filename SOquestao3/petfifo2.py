...
while pets or fila or sl_interior:
    ...
    if sl_interior:
        first = sl_interior[0]
        saida_first = first.get("entry_time", 0) + first["rest_duration"]
        
        if (saida_first <= tempo) and (first == first_anterior):
            sl_interior.pop(0)
    ...
    for i, animal in enumerate(sl_interior):
        ...
        if tempo_restante <= 0 and (i > 0 or animal != first_anterior):
            status = "ok"
        else:
            status = str(max(0, tempo_restante))
    ...
    first_anterior = sl_interior[0] if sl_interior else None
    tempo += 1
